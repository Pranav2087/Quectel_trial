...
# SimTech Ports
# -----------------------------
def get_simtech_ports():
    ports = []

    for path in glob.glob("/dev/serial/by-id/*"):
        name = os.path.basename(path)

        # Skip Sonoff Zigbee dongle
        if "usb-Itead_Sonoff_Zigbee_3.0_USB_Dongle" in name:
            continue

        # Only allow SimTech devices
        if "usb-SimTech__Incorporated_SimTech" in name:
...
def netdiag_log():
    ...
    try:
        gateway_ip = subprocess.check_output(
            f"ip addr show {IFACE_PPP} | grep peer | awk '{{print $4}}' | cut -d/ -f1",
    ...
    commands = [
        ...
        (f"ip link show {IFACE_PPP}", "PPP Link State"),
    ...
    if gateway_ip != "Not found" and gateway_ip != "":
        log_cmd(f"ping -I {IFACE_PPP} -c 2 -W 2 {gateway_ip}", log_lines, f"Ping gateway {gateway_ip}")
    else:
        log_lines.append("Ping gateway: gateway IP not found\n")
 
    log_cmd(f"ping -I {IFACE_PPP} -c 2 -W 2 8.8.8.8", log_lines, "Ping public IP via PPP")
    log_lines.append("===============================================\n")
...
def restart_connection():
    ...
    try:
        subprocess.run("sudo usb_modeswitch -R -v 1e0e -p 9001", shell=True, timeout=15)
        time.sleep(4)
        subprocess.run("sudo usb_modeswitch -R -v 1e0e -p 9011", shell=True, timeout=15)
        time.sleep(4)
    except subprocess.TimeoutExpired:
        log("⚠️ USB mode switch command timed out, continuing anyway.")

    log("Restarting PPP service...")
    try:
        subprocess.run(
            "sudo systemctl restart ppp-sim7600.service",
            shell=True,
            timeout=30
        )
        log("✅ PPP service restart command completed")
    except subprocess.TimeoutExpired:
        log("⚠️ PPP service restart timed out after 30s, continuing...")
...
def fail_safe(at_port, timeout=30):
    ...
    subprocess.run("sudo systemctl stop ppp-sim7600.service", shell=True)
    subprocess.run("sudo systemctl stop openvpn@client.service", shell=True)
    time.sleep(2)
 
    log(f"Switching Modem PID to RNDIS (9011) via {at_port}...")
    try:
        with serial.Serial(at_port, 115200, timeout=1) as ser:
            ser.write(b'at+cusbpidswitch=9011,1,1\r')
            time.sleep(0.5)
    except Exception as e:
    ...
    subprocess.run(f"sudo pkill -f 'udhcpc -i rndis0' 2>/dev/null", shell=True)
    subprocess.run(f"sudo udhcpc -i rndis0 -q -n", shell=True) 
    time.sleep(10)
 
    new_at_port = detect_at_port()
